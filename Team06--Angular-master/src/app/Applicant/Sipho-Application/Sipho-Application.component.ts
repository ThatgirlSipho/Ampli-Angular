...
@Component({
  selector: 'app-Sipho-Application',
  templateUrl: './Sipho-Application.component.html',
  styleUrls: ['./Sipho-Application.component.css']
})
export class SiphoApplicationComponent implements OnInit {
...
legalDescription: string='';
  ...
  // selectedCollectionTypes: this.fb.array([]),
    ...
    debiCheck: this.fb.group({
     ...
     averageTrackingDaysUsed: [null, [Validators.pattern(/^\d{1,8}(\.\d{1,2})?$/)]], // Pattern allows up to 10 digits before the decimal and 2 after
    }),
    eft: this.fb.group({
      monthlyValue: [null, [Validators.pattern(/^\d{1,10}(\.\d{1,2})?$/)]], // Pattern allows up to 12 digits before the decimal and 2 after
      monthlyVolume: [null, [Validators.pattern(/^\d{1,10}(\.\d{1,2})?$/)]], // Same pattern as monthlyValue
   ...
   //   avId: [null],
    ...
    allpsManagementPlatforms: this.fb.group({
     // allpsManagementPlatformId: [null],
      selectedProductId: [null],
    ...
    onEftcreditDetailSelected(event: any): void {
      const place: PlaceResult = event as PlaceResult;
      
      this.applicationForm.get('eftcreditDetail')?.patchValue({deliveryAddress:place.formatted_address});
    }
  
    onEftcreditDetailChange(event: any): void {
      const place: PlaceResult = event as PlaceResult;
      console.log('Address Changed: ', place);
    }
  ...
  populateProcessingCodes(processingCodes: any[]): void {
    ...
    processingCodes.forEach((code, index) => {
      const formGroup = this.fb.group({
        ...
        abbreviatedName: [code.abbreviatedName, [Validators.pattern(/^[a-zA-Z]{1,8}$/)]],
...
onSubmit(): void {
  ...
  if (this.applicationForm.valid) {
    ...
    alert('Please ensure all fields are in the correct format.');
    this.isSubmitting = false;
  }
}
    ...
    getUserFriendlyError(errorHtml: string): string {
      ...
      if (preTag) {
        ...
        if (rawErrorMessage) {
          ...
          } else if (rawErrorMessage.includes('File too large')) {
