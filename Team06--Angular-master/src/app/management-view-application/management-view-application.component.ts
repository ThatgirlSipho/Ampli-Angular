...
import { FormBuilder, FormGroup, FormArray, Validators, FormControl, AbstractControl, FormsModule } from '@angular/forms';
...
@Component({
  selector: 'app-management-view-application',
  templateUrl: './management-view-application.component.html',
  styleUrls: ['./management-view-application.component.css']
})
export class ManagementViewApplicationComponent {
...
documents:any[]=[];
  ...
  this.requiredDocuments.forEach(doc => {
    doc.submittedDocuments?.$values.forEach((submittedDoc:any) => {
      // Add a new form control for approvalStatus
      submittedDoc.verificationStatusControl = new FormControl(submittedDoc.verificationStatus);
    });
  });
...
get isDisabled(): boolean {
  return this.applicationStatusId !== 3;
}
...
loadRequiredDocuments() {
  this.dataService.getRequiredDocuments(this.applicationId).subscribe({
    next: (response: any) => {
      this.requiredDocuments = response.$values;
      this.checkSubmitStatus(); // Check the status after loading documents
...
// Method to request additional documents
requestDocuments(applicationId: number): void {
  ...
  this.dataService.requestAdditionalDocuments(dto).subscribe(
    response => {
      console.log('Document requests sent:', response);
      this.checkSubmitStatus(); // Disable approval after requesting documents
    },
    error => {
...
onApprovalChange(submittedDoc: any) {
  const dto = {
    submittedDocumentId: submittedDoc.submittedDocumentId,
    verificationStatus: submittedDoc.verificationStatus,
    verificationDate: new Date(),
    document:submittedDoc.document
  };

  this.dataService.adminVerifyDocument(dto).subscribe({
    next: (response) => {
...
// Method to check if submit should be disabled
checkSubmitStatus(): void {
  let anyNotApproved = false;
  let anyRequested = this.selectedDocumentIds.length > 0;

  for (let doc of this.requiredDocuments) {
    for (let submittedDoc of doc.submittedDocuments.$values) {
      if (!submittedDoc.verificationStatus) {
        anyNotApproved = true;
        break;
      }
    }
    if (anyNotApproved) break;
  }

  this.isSubmitDisabled = anyNotApproved || anyRequested;
}

// Approve the application
approveApplication() {
  this.dataService.adminApprove(this.applicationId).subscribe({
    next: (response) => {
...
returnApplication() {
  this.dataService.adminReturn(this.applicationId).subscribe({
    next: (response) => {
...
rejectApplication() {
  this.dataService.adminReject(this.applicationId).subscribe({
    next: (response) => {
...
addSubmittedDocumentControls() {
  this.requiredDocuments.forEach((doc, docIndex) => {
    doc.submittedDocuments.$values.forEach((submittedDoc: any, i: any) => {
      const controlName = `verificationStatus${submittedDoc.submittedDocumentId}`;
      if (!this.applicationForm.get(controlName)) {
        this.applicationForm.addControl(controlName, new FormControl(submittedDoc.verificationStatus || false));
      }
    });
  });
}
